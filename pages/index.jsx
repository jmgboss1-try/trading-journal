...
const css = `
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  .main-content { padding: 16px; padding-bottom: 80px; max-width: 600px; margin: 0 auto; width: 100%; }
  ...
  .pc-grid-4 { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; }
  ...
  @media (min-width: 1024px) {
    ...
    .main-content { padding: 24px 32px; padding-bottom: 24px; max-width: none; margin: 0; margin-left: 220px; width: calc(100% - 220px); }
    ...
    <div className="app-root" style={{ background: s.bg, minHeight: "100vh", color: s.text, fontFamily: "'Noto Sans KR', sans-serif", display: "flex" }}>
      ...
      <div className="sidebar">
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid " + s.border }}>
          <button onClick={() => { const drafts = {}; ACCOUNT_LIST.forEach(a => { if (capitals[a.key]) drafts[a.key] = capitals[a.key].toLocaleString("ko-KR"); }); setCapDrafts(drafts); setShowCapInput(true); }}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {ACCOUNT_LIST.map(a => (
                  <div key={a.key}>
                    ...
                    <input type="text" placeholder="예: 5,000,000 (미사용시 공란)" value={capDrafts[a.key] || ""} onChange={e => setCapDrafts(d => ({ ...d, [a.key]: e.target.value }))} />
                  </div>
                ))}
              </div>
