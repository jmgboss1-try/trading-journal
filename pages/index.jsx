...
import { LayoutDashboard, PenLine, ClipboardList, BarChart2, Bot, TrendingUp, TrendingDown, Trash2, Brain, Target, X, Save, RefreshCw, Zap, Shield, Activity, Wallet, ChevronLeft, ChevronRight, Settings } from "lucide-react";
...
const ACCOUNT_LIST = [
  { key: "국내주식",      label: "국내주식",      icon: "🇰🇷", color: "#00e5ff" },
  { key: "해외주식",      label: "해외주식",      icon: "🌍", color: "#b47aff" },
  { key: "암호화폐(현물)", label: "암호화폐 현물", icon: "₿",  color: "#ffe066" },
  { key: "암호화폐(선물)", label: "암호화폐 선물", icon: "📈", color: "#00e676" },
];
...
export default function App() {
  ...
  const saveCapitals = async () => {
    ...
    ACCOUNT_LIST.forEach(a => {
      const v = parseFloat((capDrafts[a.key] || "").replace(/,/g, "")) || 0;
      if (v > 0) parsed[a.key] = v;
    });
    setCapitals(parsed);
    try { await saveData("capitals", JSON.stringify(parsed)); } catch (e) {}
  ...
  return (
    <div style={{ background: s.bg, minHeight: "100vh", color: s.text, fontFamily: "'Noto Sans KR', sans-serif", display: "flex", flexDirection: "column" }}>
      <style>{css}</style>

      <div style={{ background: s.surface, borderBottom: "1px solid " + s.border, padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button onClick={() => {
            const drafts = {};
            ACCOUNT_LIST.forEach(a => { if (capitals[a.key]) drafts[a.key] = capitals[a.key].toLocaleString("ko-KR"); });
            setCapDrafts(drafts); setShowCapInput(true);
          }} style={{ background: s.surface2, border: "1px solid " + s.border, borderRadius: 20, padding: "4px 10px", cursor: "pointer", color: s.muted, display: "flex", alignItems: "center" }}><Settings size={12} /></button>
        </div>
      </div>
      ...
      {showCapInput && (
        <div onClick={() => setShowCapInput(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: s.surface, border: "1px solid " + s.border, borderRadius: 16, padding: 24, width: "100%", maxWidth: 380, maxHeight: "80vh", overflowY: "auto" }}>
            ...
            <div style={{ fontSize: 12, color: s.muted, marginBottom: 20 }}>각 계좌의 시작 원금을 입력하세요. 수익률 계산에 사용됩니다.</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {ACCOUNT_LIST.map(a => (
                <div key={a.key}>
                  <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                    <span>{a.icon}</span><span>{a.label}</span>
                    {capitals[a.key] && <span style={{ marginLeft: "auto", fontSize: 11, color: s.muted }}>{capitals[a.key].toLocaleString("ko-KR")}₩</span>}
                  </div>
                  <input type="text" placeholder="예: 5,000,000 (미사용시 공란)"
                    value={capDrafts[a.key] || ""}
                    onChange={e => setCapDrafts(d => ({ ...d, [a.key]: e.target.value }))} />
                </div>
              ))}
            </div>
      ...
      <div style={{ flex: 1, padding: 16, paddingBottom: 80, maxWidth: 600, margin: "0 auto", width: "100%" }}>
        {tab === "dashboard" && <DashboardTab trades={trades} setModal={setModal} capitals={capitals} totalCapital={totalCapital} pnlByCurrency={pnlByCurrency} onSetCapital={() => { const drafts = {}; ACCOUNT_LIST.forEach(a => { if (capitals[a.key]) drafts[a.key] = capitals[a.key].toLocaleString("ko-KR"); }); setCapDrafts(drafts); setShowCapInput(true); }} />}
        ...
        {tab === "settings" && <SettingsTab trades={trades} capitals={capitals} onExportJSON={exportJSON} onExportCSV={exportCSV} onImport={() => importRef.current.click()} onSetCapital={() => { const drafts = {}; ACCOUNT_LIST.forEach(a => { if (capitals[a.key]) drafts[a.key] = capitals[a.key].toLocaleString("ko-KR"); }); setCapDrafts(drafts); setShowCapInput(true); }} onClearAll={async () => { await saveTrades([]); setCapitals({}); try { await saveData("capitals", "{}"); } catch(e) {} }} />}
      </div>
      <input ref={importRef} type="file" accept=".json" onChange={importJSON} style={{ display: "none" }} />

      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: s.surface, borderTop: "1px solid " + s.border, display: "flex", zIndex: 50 }}>
        {TABS.map(t => (
      ...
      {modal && (
        <div onClick={() => setModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: s.surface, borderRadius: "16px 16px 0 0", border: "1px solid " + s.border, padding: 24, width: "100%", maxWidth: 600, maxHeight: "80vh", overflowY: "auto" }}>
...
function DashboardTab({ trades, setModal, capitals, totalCapital, pnlByCurrency, onSetCapital }) {
  ...
  const accountStats = ACCOUNT_LIST.map(a => {
    ...
    const acCap = capitals[a.key] || 0;
    const pnlByCur = acTrades.reduce((acc, t) => {
      const c = t.currency || "₩";
      acc[c] = (acc[c] || 0) + t.pnl;
      return acc;
    }, {});
    const krwPnl = pnlByCur["₩"] || 0;
    return { ...a, trades: acTrades.length, pnlByCur, wins: acWins, capital: acCap, current: acCap + krwPnl, returnPct: acCap > 0 ? (krwPnl / acCap * 100) : null };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, animation: "fadeIn 0.3s ease" }}>
      ...
      <Card accent={s.accent3} style={{ padding: 0 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
          ...
          <div style={{ padding: "10px 16px 14px", borderLeft: "1px solid " + s.border }}>
            <div style={{ fontSize: 11, color: s.muted, marginBottom: 4 }}>₩ 계좌 현재가</div>
        ...
        <div style={{ borderTop: "1px solid " + s.border, padding: "10px 16px 14px" }}>
          <div style={{ fontSize: 11, color: s.muted, marginBottom: 8 }}>통화별 손익</div>
          {Object.keys(pnlByCurrency).length === 0
            ? <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 14, color: s.muted }}>거래 기록 없음</div>
            : <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                {Object.entries(pnlByCurrency).map(([cur, val]) => (
                  <div key={cur} style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    <div style={{ fontSize: 10, color: s.muted }}>{cur}</div>
                    <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 16, fontWeight: 700, color: val >= 0 ? s.green : s.red }}>{fmt(val, cur)}</div>
                  </div>
                ))}
              </div>
          }
            ...
            <div key={a.key} style={{ background: s.surface, border: "1px solid " + s.border, borderRadius: 12, overflow: "hidden", position: "relative" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", padding: "8px 0" }}>
                <div style={{ padding: "4px 14px", borderRight: "1px solid " + s.border }}>
                  ...
                  <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, marginTop: 2 }}>{a.capital > 0 ? a.capital.toLocaleString("ko-KR") + "₩" : <span style={{ color: s.muted }}>-</span>}</div>
                </div>
                <div style={{ padding: "4px 14px", borderRight: "1px solid " + s.border }}>
                  <div style={{ fontSize: 10, color: s.muted }}>현재</div>
                  <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, marginTop: 2, color: a.capital > 0 ? (a.current >= a.capital ? s.green : s.red) : s.text }}>{a.capital > 0 ? a.current.toLocaleString("ko-KR") + "₩" : <span style={{ color: s.muted }}>-</span>}</div>
                </div>
      ...
      <Card>
        ...
        ) : <div style={{ textAlign: "center", color: s.muted, padding: "32px 0", fontSize: 13 }}>거래를 기록하면 차트가 나타납니다</div>}
      </Card>
...
function RecordTab({ onAdd }) {
  ...
  const scanImage = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, messages: [{ role: "user", content: [{ type: "image", source: { type: "base64", media_type: validMime, data: imgBase64 } }, { type: "text", text: prompt }] }] }) });
...
function AITab({ trades }) {
  ...
  const analyzeChart = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, messages: [{ role: "user", content: [{ type: "image", source: { type: "base64", media_type: validChartMime, data: chartBase64 } }, { type: "text", text: prompt }] }] }) });
      const data = await res.json();
  ...
  const requestAI = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, messages: [{ role: "user", content: PROMPTS[aiType] + buildSummary() + (question ? "\n\n추가질문: " + question : "") }] }) });
      const data = await res.json();
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 12 }}>
          {ACCOUNT_LIST.map(a => (
            <div key={a.key} style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px", background: s.surface2, borderRadius: 8, fontSize: 13 }}>
              ...
              <span style={{ fontFamily: "'JetBrains Mono',monospace", color: capitals[a.key] ? s.accent : s.muted }}>
                {capitals[a.key] ? capitals[a.key].toLocaleString("ko-KR") + "₩" : "미설정"}
              </span>
            </div>
          ))}
        </div>
