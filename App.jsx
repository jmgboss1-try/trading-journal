...
function CardHeader({ children, className = "" }) { return <div className={`p-5 md:p-6 ${className}`}>{children}</div>; }
function CardTitle({ children, className = "" }) { return <h2 className={`text-xl font-semibold ${className}`}>{children}</h2>; }
function CardContent({ children, className = "" }) { return <div className={`px-5 pb-5 md:px-6 md:pb-6 ${className}`}>{children}</div>; }
...
function Input({ className = "", ...props }) { return <input className={`w-full rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 shadow-sm outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30 ${className}`} {...props} />; }
function Textarea({ className = "", ...props }) { return <textarea className={`w-full rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 shadow-sm outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30 ${className}`} {...props} />; }
function Badge({ children, className = "" }) { return <span className={`inline-flex items-center rounded-full bg-slate-800 px-2.5 py-1 text-xs text-slate-200 ${className}`}>{children}</span>; }
function Field({ label, children }) { return <label><span className="mb-1 block text-sm font-medium text-slate-300">{label}</span>{children}</label>; }
function StatCard({ title, value, emoji }) { return <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-3"><div className="mb-1 flex items-center gap-2 text-xs text-slate-400"><span>{emoji}</span><span>{title}</span></div><div className="text-lg font-semibold text-slate-100">{value}</div></div>; }
function MetricCard({ label, value, tone = "cyan" }) {
  ...
  return <div className={`rounded-2xl border p-4 ${tones[tone]}`}><div className="text-xs opacity-80">{label}</div><div className="mt-1 text-lg font-semibold">{value}</div></div>;
}
...
export default function TradingJournalApp() {
  ...
  const filtered = useMemo(() => {
    const q = queryStringNormalize(query);
  ...
  async function saveEntry() {
    ...
    if (user && isFirebaseConfigReady(firebaseConfig)) {
      try {
        ...
        const { db } = getFirebaseServices(firebaseConfig);
        await setDoc(doc(db, "users", user.uid, "entries", nextEntry.id), {
          ...nextEntry,
          userId: user.uid,
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(34,211,238,0.16),_transparent_28%),linear-gradient(180deg,#07111f_0%,#0b1324_45%,#111827_100%)] p-3 text-slate-100 md:p-6">
          ...
          <Card>
            <CardHeader>
              ...
              <div className="mt-4 grid gap-3">
                <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="종목, 셋업, 태그 검색" />
                <div className="flex flex-wrap gap-2">
                ...
                {filtered.map((entry) => (
                  ...
                  >
                    ...
                    <div className="mt-1 flex flex-wrap gap-2 text-xs text-slate-300">
                      <span>{entry.side}</span><span>·</span><span>{entry.setup}</span><span>·</span><span>{entry.status}</span>
            ...
            <CardHeader>
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => deleteEntry(form.id)}>삭제</Button>
            ...
            <CardContent className="space-y-6">
              ...
              <section>
                ...
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  ...
                  <Field label="자산군">
                    <select className="w-full rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30" value={form.category} onChange={(e) => updateForm("category", e.target.value)}>
                  ...
                  <Field label="방향">
                    <select className="w-full rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30" value={form.side} onChange={(e) => updateForm("side", e.target.value)}>
                      <option value="Long">Long</option><option value="Short">Short</option>
                    </select>
                  </Field>
                  <Field label="셋업">
                    <select className="w-full rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30" value={form.setup} onChange={(e) => updateForm("setup", e.target.value)}>
                      <option value="반등매매">반등매매</option><option value="돌파매매">돌파매매</option><option value="눌림목">눌림목</option><option value="추세추종">추세추종</option><option value="역추세">역추세</option>
                    </select>
                  </Field>
                  <Field label="타임프레임">
                    <select className="w-full rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30" value={form.timeframe} onChange={(e) => updateForm("timeframe", e.target.value)}>
                  ...
                  <Field label="상태">
                    <select className="w-full rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30" value={form.status} onChange={(e) => updateForm("status", e.target.value)}>
                      <option value="대기">대기</option><option value="진행중">진행중</option><option value="종료">종료</option>
                    </select>
                  </Field>
                  <Field label="스토캐스틱">
                    <select className="w-full rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30" value={form.stochastic} onChange={(e) => updateForm("stochastic", e.target.value)}>
                      <option value="과매도">과매도</option><option value="중립">중립</option><option value="과매수">과매수</option>
                    </select>
                  </Field>
                  <Field label="시장 상태">
                    <select className="w-full rounded-2xl border border-slate-700/70 bg-slate-900/70 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30" value={form.marketCondition} onChange={(e) => updateForm("marketCondition", e.target.value)}>
                      <option value="상승">상승</option><option value="하락">하락</option><option value="횡보">횡보</option>
                    </select>
                  </Field>
                </div>
              </section>
